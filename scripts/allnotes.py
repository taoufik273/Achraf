...
def telecharger_allnotes():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM 'index2'")
        ...
        def copier_entete(ws: Worksheet, source_row: int, target_row: int, page_num: int):
            ...
            ws.cell(row=target_row + 32, column=10, value=membre)  # J33 de chaque page
            ws.cell(row=target_row + 32, column=6, value=membre1)  # F33 de chaque page
        ...
        for idx, row in notes_df.iterrows():
            if ligne > 30 + 37 * (page_global_num - 1) or row.iloc[3] != prev_b7 or row.iloc[1] != prev_g7 or row.iloc[2] != prev_k7:
                if ligne > 11 + 37 * (page_global_num - 1):  # Vérifie si des données ont été ajoutées
                    ...
                    ws.cell(row=33 + 37 * (page_global_num - 1), column=10, value=membre)
                    ws.cell(row=33 + 37 * (page_global_num - 1), column=6, value=membre1)
