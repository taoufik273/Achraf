...
input_excel_path = get_absolute_path('..', 'INPUT', 'liste_A3.xlsx')
output_excel_path = get_absolute_path('..', 'OUTPUT', 'liste_remplie.xlsx')
...
# Rename the active sheet to "LISTES"
sheet.title = "LISTES"
...
cursor.execute("SELECT jour, numliste, heure, ordre, massar, nom, serie FROM liste ORDER BY jour, heure, numliste")
liste_data = cursor.fetchall()

# Organize data by jour, numliste, and heure
organized_data = {}
for row in liste_data:
    jour, numliste, heure, ordre, massar, nom, serie = row
...
if pdf_path:
    ...
    messagebox.showinfo("ممتاز", f"تم تحميل لوائح المترشحين بنجاح")
else:
