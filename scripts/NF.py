...
def telecharger_NF():
    try:
        ...
        def copier_entete(ws: Worksheet, source_row: int, target_row: int, page_num: int):
            ...
            ws.cell(row=target_row + 39, column=7, value=membre)  # G40 de chaque page
            ws.cell(row=target_row + 39, column=4, value=membre1)  # D40 de chaque page
        ...
        for idx, row in notes_df.iterrows():
            if ligne > 36 + 43 * (page_global_num - 1) or row.iloc[3] != prev_b10 or row.iloc[1] != prev_f10 or row.iloc[2] != prev_h10:
                if ligne > 13 + 43 * (page_global_num - 1):  # Vérifie si des données ont été ajoutées
                    ...
                    ws.cell(row=40 + 43 * (page_global_num - 1), column=7, value=membre)
                    ws.cell(row=40 + 43 * (page_global_num - 1), column=4, value=membre1)
